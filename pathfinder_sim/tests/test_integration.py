...
def game_environment():
    ...
    Set up a basic game environment:
      - A 10x10 map with some terrain.
      - A global TurnManager using the RulesEngine and the map.
      - Two characters: Alice and Bob.
    """
    # Create a map and set some terrain.
    test_map = Map(10, 10)
    # Example: Set column 3 (rows 3-5) as difficult terrain.
    for y in range(3, 6):
...
def test_full_turn_simulation_move_and_attack(game_environment):
    """
    Test a full turn where Alice moves and then attacks Bob.
    Verify that:
      - Alice’s position updates correctly after the move.
      - The attack is processed using her updated position.
    """
    ...
    attack_action = AttackAction(actor=characters["Alice"], defender=characters["Bob"], 
                                 weapon_bonus=2, weapon=None, is_touch_attack=False, target_flat_footed=False,
    ...
    # Verify that Alice's move action updated her position to (2,0).
    ...
    # Check that an attack result was returned.
...
def test_full_turn_simulation_spell_and_swift(game_environment):
    """
    Test a full turn where Bob casts a spell and takes a swift action.
    Verify that:
      - The spell action produces a valid result.
    ...
    spell_action = SpellAction(actor=characters["Bob"], target=characters["Alice"], spell_name="Magic Missile", 
                               action_type=ActionType.STANDARD)
    swift_action = SkillCheckAction(actor=characters["Bob"], skill_name="Use Magic Device", dc=15, 
...
def test_multi_turn_condition_update(game_environment):
    ...
    For example, Bob has a Blinded condition for 2 rounds.
    """
    env = game_environment
    tm = env["turn_manager"]
    characters = env["characters"]
    ...
    # Process two turns and tick conditions.
    for _ in range(2):
        turn = tm.new_turn()
...
def test_json_order_parsing_integration(game_environment):
    ...
    then check that TurnManager parses the orders correctly.
    """
    env = game_environment
    tm = env["turn_manager"]
    characters = env["characters"]
...
def test_action_limit_enforcement(game_environment):
    """
    Test that if a character attempts to exceed allowed actions, an exception is raised.
    For instance, adding a standard action after two move actions.
    """
    env = game_environment
    tm = env["turn_manager"]
    characters = env["characters"]
...
def test_obstacle_movement_interaction():
    """
    Test a turn where the path is completely blocked.
    """
    from movement import Map, MovementAction
    m = Map(3, 3)
    for x in range(3):
...
def test_full_round_action_charge(game_environment):
    ...
    Verify that the combined movement and attack are processed.
    """
    env = game_environment
    tm = env["turn_manager"]
    characters = env["characters"]
    ...
    alice = characters["Alice"]
    bob = characters["Bob"]
    # Pass actual character object for defender instead of a string.
    full_round_action = FullRoundAction(actor=alice, parameters={"type": "charge", "target": (3, 0), "defender": bob, "weapon": None}, action_type=ActionType.FULL_ROUND)
...
def test_integration_spell_and_movement(game_environment):
    """
    Simulate a scenario where a character moves and then a spell is cast using the updated position.
    Verify that:
      - Movement updates the character's position.
      - The spell resolver processes the spell based on the new position.
    """
    env = game_environment
    tm = env["turn_manager"]
    characters = env["characters"]
    ...
    assert move_result["final_position"] == (4, 0)
...
def test_global_state_consistency(game_environment):
    """
    After processing a turn with multiple actions, verify that the global state
    ...
    attack_action = AttackAction(actor=alice, defender=bob, weapon_bonus=2, weapon=None,
                                 is_touch_attack=False, target_flat_footed=False, action_type=ActionType.STANDARD)
    spell_action = SpellAction(actor=bob, target=alice, spell_name="Magic Missile", action_type=ActionType.STANDARD)
    turn.add_action(move_action)
    turn.add_action(attack_action)
    turn.add_action(spell_action)
    ...
    tm.process_turn(turn)
    # Check that Alice's position is updated.
    ...
    # Conditions should have decreased; ensure at least one condition's duration has reduced.
