...
class Character(TypedDict):
    ...
    intentions: str       # e.g. "protect the village"
    motivation: str       # e.g. "seeks redemption"

class StoryContinuation(dspy.Signature):
    """
    You are the epic narrator of a high-fantasy campaign. Your task is to generate the next narrative segment of the story.

    Your Task:
        1. Set the next scene in vivid, sensory detail: sights, sounds, smells, and atmosphere.
        2. Emphasize heroic stakes or looming dangers tied to the party’s goals.
        3. Never speak or act for player characters or override their choices.
        4. Describe the outcome of any action_results in a clear and engaging manner.
        5. You may sometimes include brief player character surface thoughts or reactions for flavor. Only do this if it adds to the story.
        6. Follow story_guidelines and never contradict lore_context.
        7. Make sure the story is compelling, eloquent, addictive, heroic, immersive, poignant...
        8. End with a compelling hook or choice to invite the players’ next move.

    Constraints:
      - Never take actions on behalf of players or speak for them.
      - When a player’s action completes (action_results), describe its outcome.
      - You may include brief NPC surface thoughts or reactions for flavor.
      - Honor story_guidelines and lore_context—do not contradict them.
    """
    user_message: str = dspy.InputField(
        desc="The player's latest cleaned-up action intent or dialogue"
    )
    previous_messages: List[str] = dspy.InputField(
        desc="Recent messages for dialog flow and context"
    )
    story_guidelines: str = dspy.InputField(
        desc="Game style and tone, story and arch development, clues about what's happening behind the scenes, etc."
    )
    lore_context: List[str] = dspy.InputField(
        desc="Immutable world lore and context snippets. Historical info."
    )
    action_results: List[ActionResult] = dspy.InputField(
        desc="Resolved skill/item check outcomes. Intent success or failure and margin definition."
    )
    characters: List[Character] = dspy.InputField(
        desc="Characters present with their roles. For NPCs: intent and motivation of the character."
    )

    continuation: str = dspy.OutputField(
        desc="The next narrative segment: describe outcomes, environment, NPC reactions; do not control PCs"
    )